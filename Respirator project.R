...
#	v_cov<-read.table("covariates.txt", header=T)
#	v_class<-read.table("classifiers.txt", header=T)
#	dat_sym<-read.table("Symmetrized PR data.txt", header=T)
#	dat_pr<-read.table("nonsymm PR data.txt", header=T)
#	synd_list<-read.table("synd_list.txt", header=T)
#	synd_mns<-read.table("synd_mns.txt", header=T)



#v_cov<-read.table("Z:/Hallgrimsson/Collaborations/FaceBase/FaceBase 2/Analysis/2019/Analysis Data/02 Max Sample w Relatives/covariates.txt", header=T)
#v_class<-read.table("Z:/Hallgrimsson/Collaborations/FaceBase/FaceBase 2/Analysis/2019/Analysis Data/02 Max Sample w Relatives/classifiers.txt", header=T)
#dat_sym<-read.table("Z:/Hallgrimsson/Collaborations/FaceBase/FaceBase 2/Analysis/2019/Analysis Data/02 Max Sample w Relatives/PR data sym corrected 20yo.txt", header=T)
#dat_pr<-read.table("Z:/Hallgrimsson/Collaborations/FaceBase/FaceBase 2/Analysis/2019/Analysis Data/02 Max Sample w Relatives/PR data full not corrected.txt", header=T)
#v_data<-read.table("Z:/Hallgrimsson/Collaborations/FaceBase/FaceBase 2/Analysis/2019/Analysis Data/02 Max Sample w Relatives/raw lms.txt", header=T)



#Data standardized for age and sex

#setwd("Y:/Hallgrimsson/Collaborations/FaceBase/FaceBase 2/Analysis/2019/Analysis Data/04 Min Synd n 10")
...
v_data<-read.table("raw data no outliers.txt", header=T)

data_raw <- read.csv("Data for Analysis 2020.csv", header = T)
v_pr2d<-v_data[,18:212]

#Some corrections
v_class$Syndrome.Category[which(v_class$Syndrome.Category=="Uncategorizedv")]<-"Uncategorized"
v_class$Syndrome.Category[which(v_class$Syndrome.Category=="cartilate")]<-"cartilage"
v_class$Syndrome.Category[which(v_class$Syndrome.Category=="cartilage")]<-"Cartilage"
v_class$Syndrome.Category[which(v_class$Syndrome=="Osteogenesis Imperfecta")]<-"Osteogenesis imperfecta"	
#Data standardized for age and sex

#v<-read.table("data without outliers.txt", header=T)
#v_data<-v[,24:218]
...
# Put in landmark colnames

lm_names <- colnames(data_raw)[27:222]


# Procrustes superimposition
v_array<-arrayspecs(v_pr2d, lms,coords)
v_pr<-procSym(v_array, pairedLM=pairedLM)
v_pr2d<-two.d.array(v_pr$rotated)
...
Age2<-v_cov$Age^2
Age3<-v_cov$Age^3	
dat_pr3d<-arrayspecs(dat_pr, lms, coords)
pr_gm_s<-geomorph.data.frame(shape=dat_pr3d, Age=v_cov$Age, Age_2=Age2,Age_3=Age3, Sex=v_class$Sex, Race=v_class$Race, Race2= v_class$Race2, CS=v_cov$CS, Ctr_rel_synd=v_class$Synd_rel_ctr, Ctr_Synd=v_class$Ctr_Syndrome)

ac_pr2 <- procD.lm(shape ~ Age + Age_2 + Age_3 +Sex, data=pr_gm_s, iter=1)
...
#	v<-read.table("data without outliers.txt", header=T)
v<-read.csv("method_avgs_imp.csv", header=T)
v_data<-v[,18:212]

# Remove a couple of CS outliers

v<-v[-which(v$CS<200),]
v<-v[-which(v$CS>610),]	


#v<- v[-which(v$Subject.ID=="LA0154_1"),]
v<- v[-which(v$Subject.ID=="LA0153_1"),]	
v<- v[-which(v$Subject.ID=="AB0854_1"),]	
LA0156_1
...
mean_face_lm <- mshape(arrayspecs(v_data, lms, coords)) 
